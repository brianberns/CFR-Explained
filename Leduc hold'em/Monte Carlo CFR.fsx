...
module LeducCfrTrainer =
    ...
    let private cfr infoSetMap playerCards communityCard updatingPlayer =
    ...
    let train numIterations =
        ...
        let utilities, infoSetMap =
            ...
            (Map.empty, Seq.indexed deals)
                ||> Seq.mapFold (fun infoSetMap (i, (playerCards, communityCard)) ->

                        // evaluate one game starting with this deal
                    let utility, keyedInfoSets =
                        let updatingPlayer = i % LeducHoldem.numPlayers
                        cfr infoSetMap playerCards communityCard updatingPlayer
...
let run () =
    ...
    printfn $"Running Leduc Hold'em Monte Carlo CFR for {numIterations} iterations\n"
